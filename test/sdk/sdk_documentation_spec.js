...
const expectedFinalJsonFilesDir = path.resolve(
  __dirname,
  "resources/expected_final_json_files"
)
const baseURI = path.resolve(
  __dirname,
  testDir + "/src/webide-plugin-example/service/Sample.json"
)
const sourceFilesExcludePattern = "\\w+\\.json"
const exampleFilesIncludePattern = /.+\\test\\\w+.js/

function eraseFilesFromSystem(filesPath) {
  const internalSdkJsonPath = path.resolve(
    __dirname,
    filesPath + "/" + sdk.test.internalJsonFileName
  )
  const publicSdkJsonPath = path.resolve(
    __dirname,
    filesPath + "/" + sdk.test.publicJsonFileName
  )
  ...
  try {
    ...
    console.error(
      "Error: " + internalSdkJsonPath + " was NOT deleted\n\t" + error.message
    )
  }
  ...
  try {
    ...
    console.error(
      "Error: " + publicSdkJsonPath + " was NOT deleted\n\t" + error.message
    )
  }
}

function readFiles(readPath) {
  const internalJsonPath = path.resolve(
    __dirname,
    readPath + "/" + sdk.test.internalJsonFileName
  )
  const publicJsonPath = path.resolve(
    __dirname,
    readPath + "/" + sdk.test.publicJsonFileName
  )
  return {
    internalJson: JSON.parse(fs.readFileSync(internalJsonPath, "utf8")),
    publicJson: JSON.parse(fs.readFileSync(publicJsonPath, "utf8"))
  }
}
                    ...
                    notificationObject: {
                      ...
                      parameterProperties: {
                        notificationCount: {
                          ...
                          type: "number",
                          description:
                            "a notification count to start the service from."
                        }
                      },
                      description: "notification object to organize the count"
                    }
  ...
  describe("happy path", () => {
    describe("unit tests", () => {
      ...
      it("tests that the Interfaces object was built for internal API", () => {
        ...
        const expectedInterfacesInternal = _.cloneDeep(
          testsData.interfacesInternal
        )
        ...
        expect(actualInterfacesInternal).to.deep.equal(
          expectedInterfacesInternal
        )
      })
            ...
            methods: {
              ...
              getNotificationCount: {
                ...
                returns: {
                  type: "number",
                  description:
                    "Number of greeting notifications displayed so far"
                },
                visibility: "public",
                example:
                  '      var sampleService = getService("sample")\r\n\r\n      return sampleService.getNotificationCount().then(function(notiCount) {\r\n        expect(notiCount).to.equal(0)\r\n      })'
      ...
      it("tests that only public and internal modifiers are filtered", () => {
        ...
        interfacesMeta[
          "sap.webide.example.plugin.service.Sample"
        ].methods.sayHello.visibility =
          "internal"
        interfacesMeta[
          "sap.webide.example.plugin.service.Sample"
        ].methods.getNotificationCount.visibility =
          "private"
        const expectedFilterObjectPublic = {
          modifyVar:
            interfacesMeta["sap.webide.example.plugin.service.Sample"].methods
              .modifyVar
        }
        ...
        const actualFiltredObjectsPublic = sdk.test.filterPropertiesByVisibility(
          interfacesMeta["sap.webide.example.plugin.service.Sample"].methods,
          [_PUBLIC]
        )
        const actualFiltredObjectsInternal = sdk.test.filterPropertiesByVisibility(
          interfacesMeta["sap.webide.example.plugin.service.Sample"].methods,
          [_PUBLIC, _INTERNAL]
        )

        expect(actualFiltredObjectsPublic).to.deep.equal(
          expectedFilterObjectPublic
        )
        expect(actualFiltredObjectsInternal).to.deep.equal(
          expectedFilterObjectInternal
        )
      })
    })
    ...
    describe("end to end", () => {
      it("tests the whole scenerio for internal and public API", () => {
        ...
        ).to.deep.equal(expectedPublicWebIdeJson)
        expect(
          publicAndInternalWebIdeJsonObject.internalWebIdeJson
        ).to.deep.equal(expectedInternalWebIdeJson)
      })
      ...
      describe("Whole scenerion using files", () => {
        ...
        it("tests the generating of sdk from source files and example files to output files", () => {
          ...
          const actualJsonFiles = readFiles(outDir)
          const expectedJsonFiles = readFiles(expectedFinalJsonFilesDir)
  ...
  describe("restrictions on source, example and metadata files", () => {
    describe("source and example files i/o issues", () => {
      it("throws an error when there are no source files or path doesn't exist", () => {
        const getApiJsonFunction = () =>
          sdk.test.getApiJson(
            path.resolve(
              __dirname,
              testDir + "/src/webide-plugin-example/i18n/fake"
            ),
            outDir,
            sourceFilesExcludePattern
          )
      ...
      it("throws an error when there are no example files or path doesn't exist", () => {
        const actualExamples = () =>
          sdk.test.generateExamplesJson(
            path.resolve(__dirname, testDir + "/fake"),
            exampleFilesIncludePattern,
            {}
          )

        expect(actualExamples).to.throw(
          "Error: The path given to the Examples files: " +
            path.resolve(__dirname, testDir + "/fake does not exists")
        )
      })
      ...
      describe("log warnings", () => {
        ...
        it("logs a warn message when the api.json and symbols-pruned-ui5.json (temp files) don't exist", () => {
          const fakeOutDir = path.resolve(
            __dirname,
            "resources/testfeature/dist/fake"
          )
          const fakeAPIPath = path.resolve(__dirname, fakeOutDir + "/api.json")
          const fakeSymbolsPath = path.resolve(
            __dirname,
            fakeOutDir + "/symbols-pruned-ui5.json"
          )
          const eraseFilesFromSystem = () =>
            sdk.test.eraseFilesFromSystem(fakeOutDir)

          expect(eraseFilesFromSystem).to.not.throw()
          expect(console.warn).to.have.been.called
          expect(consoleWarnSpy.args[0][0]).to.equal(
            "temp SDK file: " +
              fakeAPIPath +
          ...
          expect(consoleWarnSpy.args[1][0]).to.equal(
            "temp SDK file: " +
              fakeSymbolsPath +
              " was NOT deleted\n\tENOENT: no such file or directory, unlink '" +
              fakeSymbolsPath +
    ...
    describe("restrictions on source, example and metadata files format", () => {
      it("throws an error when the source files have a service and a class with the same name", () => {
        ...
        const actualJSDocInternal = () =>
          sdk.test.generateSdkJSDoc([_PUBLIC, _INTERNAL], apijson)

        expect(actualJSDocInternal).to.throw(
          "Service and class with the same name exist, this could lead to inconsistencies in SDK navigation and in the service and class examples: sample"
        )
      })
      ...
      it("throws an error when a metadata object has a public method that is not included in the example files", () => {
        ...
        const actualInterfacesPublic = () =>
          sdk.test.generateSdkInterfaces(interfacesMeta, {}, true, [_PUBLIC])

        expect(actualInterfacesPublic).to.throw(
          "events: Interfaces.sap.webide.example.plugin.service.Sample.notificationDisplayed is public and does not have an example"
        )
      })
      ...
      it("throws an error when a metadata object has a method with a return type of string", () => {
        ...
        interfacesMeta[
          "sap.webide.example.plugin.service.Sample"
        ].methods.getNotificationCount.returns =
          "string"
        ...
        expect(actualInterfacesPublic).to.throw(
          "'returns' must be an object and not a string. Interfaces.sap.webide.example.plugin.service.Sample -> getNotificationCount"
        )
      })
      ...
      it("throws an error when a metadata object has no description", () => {
        ...
        expect(actualInterfacesPublic).to.throw(
          "interface sap.webide.example.plugin.service.Sample is missing a description"
        )
      })
      ...
      it("throws an error when a metadata object has a method with an example but no description", () => {
        ...
        interfacesMeta[
          "sap.webide.example.plugin.service.Sample"
        ].methods.getNotificationCount.description = undefined
        const examples = _.cloneDeep(testsData.examples)

        const actualInterfacesInternal = () =>
          sdk.test.generateSdkInterfaces(interfacesMeta, examples, true, [
            _PUBLIC,
            _INTERNAL
          ])

        expect(actualInterfacesInternal).to.throw(
          "method getNotificationCount from interface sap.webide.example.plugin.service.Sample is missing a description"
        )
      })
      ...
      it("throws an error when a metadata public object has no component", () => {
        ...
        const actualInterfacesPublic = () =>
          sdk.test.generateSdkInterfaces(interfacesMeta, {}, true, [
            _PUBLIC,
            _INTERNAL
          ])

        expect(actualInterfacesPublic).to.throw(
          "interface sap.webide.example.plugin.service.Sample does not have a component defined"
        )
      })
      ...
      it("throws an error when a public service doesn't have an example", () => {
        ...
        const actualServiciesPublic = () =>
          sdk.test.generateSdkServices(
            servicesMetadata,
        ...
        expect(actualServiciesPublic).to.throw(
          "Public service sample is missing an example"
        )
      })
      ...
      it("throws an error when a metadata object has a an example, with type not allowed by object", () => {
        ...
        expect(actualInterfacesPublic).to.throw(
          "Examples with type: fakeAttribute, are not allowed for: Interfaces.sap.webide.example.plugin.service.Sample"
        )
      })
      ...
      it("throws an error when a service or class file has @example tag in it", () => {
        ...
        const actualJSDocInternal = () =>
          sdk.test.generateSdkJSDoc([_PUBLIC, _INTERNAL], apijson, true)

        expect(actualJSDocInternal).to.throw(
          "Error: @example is not allowed, found in: Service.sample.methods.getNotificationCount"
        )
      })
      ...
      describe("non strict mode console log errors", () => {
        ...
        it("logs an error message when in non strict mode and a metadata class object has a public method without an example", () => {
          ...
          expect(consoleErrorSpy.args[0][0]).to.equal(
            "Taking example for Classes.helloUtils.method1 from jsdoc. Note: this will soon be removed. Please move the example to a test file."
          )
          expect(consoleErrorSpy.args[1][0]).to.equal(
            "Taking example for Classes.helloUtils.method2 from jsdoc. Note: this will soon be removed. Please move the example to a test file."
          )
        })
