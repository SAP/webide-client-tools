    ...
    internal: {
        ...
        createRequireJSOptimizeOptions: function createRequireJSOptimizeOptions(
            ...
            const pluginsNamesAndRelativePaths = _.mapValues(
                ...
                currAbsPath =>
                    utils.backslashesToSlashes(
                        path.relative(pathPrefix, currAbsPath)
                    )
            )
            ...
            const expandedFilesNoSuffix = _.map(
                expandedFiles,
                currExpandedFile => currExpandedFile.replace(/.js$/, "")
            )
            ...
            const expandedFilesWithDotPrefixNames = _.map(
                ...
                currExpandedFile => {
                    const matchingPair = _.find(
                        ...
                        currNameAndUri => {
                            ...
                            return assertedCurrExpandedFile.startsWith(
                                currNameAndUri[1]
                            )
                        }
                    )
                 ...
                 * */
                    ...
                    return currExpandedFile.replace(
                        currPluginUri,
                        currPluginDotName
                    )
                }
         ...
         *    @see http://requirejs.org/docs/optimization.html#options for details.
         */
        bundleJavaScriptSources: function bundleJavaScriptSources(
            target,
            options
        ) {
            ...
            return optimizeAsync(config).then(() => {
                const validationMessages = _.reduce(
                    config.modules,
                    (result, currModule) => {
                        const orgTarget = path.resolve(
                            config.dir,
                            `${currModule.name}.js`
                        )
                        const bundledContents = fs.readFileSync(
                            orgTarget,
                            "UTF-8"
                        )
                        ...
                        if (errorLocations.length > 0) {
                            const errorLocationsText = _.map(
                                errorLocations,
                                JSON.stringify
                            )
        ...
        bundleI18n: function bundleI18n(target, options) {
            ...
            const bundledPluginNamesByLength = utils.sortByLength(
                bundledPluginNames
            )
            const bundledI18nResourcesRealPathsAndNames = _.map(
                ...
                currI18nResource => {
                    const pluginNameOfI18nResource = _.find(
                        ...
                        currPluginName =>
                            currI18nResource.startsWith(currPluginName)
                    )
                    const pluginUri =
            ...
            const aI18nConfigs = _.map(
                ...
                currPathAndName => {
                    ...
                    if (!fs.existsSync(currI18nPath)) {
                        ...
                        const sKey = `${currI18nOrgKey.replace(
                            /\./g,
                            "/"
                        )}.properties`
                        const escapedI18Text = stringEscape(
                            i18nTextWithDatePrefix
                        )
        ...
        createMetadataJson: function createMetadataJson(target, options) {
            ...
            function transformPluginNameAndRemoveBaseURI(obj) {
                return _.reduce(
                    obj,
                    (result, oldValue) => {
                        const modifiedBaseURI = oldValue.baseURI.replace(
                            /\\/g,
                            "/"
                        )
                        const newPluginName =
                            actualOptions.pluginsPrefix +
                            path
                                .relative(pkgPath, modifiedBaseURI)
                                .replace(/\\/g, "/")
            ...
            const transformedConfigsToInterfaceMeta = _.mapValues(
                ...
                currInterfacesToJson =>
                    transformInterfaces(currInterfacesToJson)
            )
        ...
        bundleCachePackageWrapper(orgPkgContents, outDir, cacheFolderName) {
            ...
            const wrapperStringContents = JSON.stringify(
                wrapperPkgObj,
                null,
                "\t"
            )
